...
sayHelloButton.addEventListener('mouseout', undoColor);
...
const ohMy = () => {
    ...
    .then(res => {
        ...
        // res.data.forEach(res => document.createElement('p').textContent = res.data[i])
...
/*
    ...
    1: Send no queries on the URL -- what happened? 'YOU SENT AN EMPY QUERY'
    ...
    queries: {hello: 'haii', hai: 'hello'}
    [[Prototype]]: Object
    ------
*/

let getRequest = () => {
    axios.get('http://localhost:3000/query-test/?hello=haii&hai=hello')
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small form and write a function that makes a post request. Then
    
    you'll attach that function to the submit event on the form. We'll be creating a list of foods. 
